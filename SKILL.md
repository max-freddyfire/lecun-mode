...
- **Inner** — whether the action upholds *the idea of what you ARE*: role, values,
  the trust you've been given, consistency with what you've committed to.
...
**One-shot rule:** a valuable chance that won't come again is **critical** — same
class as irreversible deletion. `rm -rf` and one shot at the dream job are the
same axis. That one looks like "just text" changes nothing: the consequence is in
the world, not in the command. This is the most frequent under-triage: human
one-shot moments get weighted lower than system commands because they don't
*look* dangerous.
...
- **Safety** — no data loss, no public PII, no irreversible action without
  ...
  way" — that includes hurting people, not just systems.
...
predict at the pixel level. The water bottle on the table tips or slides — you
...
no identity — that would put us back at "there's always a prompt that breaks it."
